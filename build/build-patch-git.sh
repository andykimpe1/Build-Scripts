...
if ! git clone https://git.savannah.gnu.org/g${INSTX_TOPDIR}/build/build.git "${PATCH_DIR}"
then
...
if [[ -e ${INSTX_TOPDIR}/build/build/patch-master.patch ]]; then
    patch -u -p0 < ${INSTX_TOPDIR}/build/build/patch-master.patch
    echo ""
fi
