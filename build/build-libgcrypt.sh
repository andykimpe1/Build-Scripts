...
if [[ -e ${INSTX_TOPDIR}/patch/libgcrypt.patch ]]; then
    echo ""
    echo "****************************"
    echo "Patching package"
    echo "****************************"

    patch -u -p0 < ${INSTX_TOPDIR}/patch/libgcrypt.patch
fi

is_apple_m1=$(sysctl machdep.cpu.brand_string 2>&1 | grep -i -c 'Apple M1')
if [[ "${is_apple_m1}" -eq 1 ]]; then
    if [[ -e ${INSTX_TOPDIR}/patch/libgcrypt-darwin.patch ]]; then
        echo ""
        echo "****************************"
        echo "Patching package (Darwin)"
        echo "****************************"

        patch -u -p0 < ${INSTX_TOPDIR}/patch/libgcrypt-darwin.patch
    fi
fi
