...
if [[ "$INSTX_CXX14" -eq 1 ]]
then
    if ! ${INSTX_TOPDIR}/build/build-nghttp2.sh
    then
...
if ! ${INSTX_TOPDIR}/build/build-openldap.sh
then
...
if [[ -e ${INSTX_TOPDIR}/patch/curl.patch ]]; then
    echo ""
    echo "**********************"
    echo "Patching package"
    echo "**********************"

    patch -u -p0 < ${INSTX_TOPDIR}/patch/curl.patch
fi
