...
if [[ "$IS_DARWIN" -ne 0 ]]
then
    if ! ${INSTX_TOPDI${INSTX_TOPDIR}/build/build-iconv-utf8mac.sh
    then
        echo "Failed to build iConv and GetText (1st time)"
        exit 1
    fi
else
    if ! ${INSTX_TOPDI${INSTX_TOPDIR}/build/build-iconv.sh
    then
        echo "Failed to build iConv and GetText (1st time)"
        exit 1
    fi
...
if ! ${INSTX_TOPDI${INSTX_TOPDIR}/build/build-gettext.sh
then
