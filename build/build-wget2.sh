...
if ! ${INSTX_TOPDI${INSTX_TOPDIR}/build/build-pcre2.sh
then
...
if ! ${INSTX_TOPDI${INSTX_TOPDIR}/build/build-openssl.sh
then
...
#if ! ${INSTX_TOPDI${INSTX_TOPDIR}/build/build-libhsts.sh
#then
...
then
    if ! ${INSTX_TOPDI${INSTX_TOPDIR}/build/build-flex.sh
    then
...
then
    if ! ${INSTX_TOPDI${INSTX_TOPDIR}/build/build-grep.sh
    then
...
then
    if ! ${INSTX_TOPDI${INSTX_TOPDIR}/build/build-lzip.sh
    then
...
then
    ...
    if [ "$ver" -ge 27 ]
    then
        if ! ${INSTX_TOPDI${INSTX_TOPDIR}/build/build-libpsl.sh
        then
...
if [[ -e ${INSTX_TOPDIR}/build/build/wget2.patch ]]; then
    echo ""
    echo "**************************"
    echo "Patching Wget2"
    echo "**************************"

    patch -u -p0 < ${INSTX_TOPDIR}/build/build/wget2.patch
fi
