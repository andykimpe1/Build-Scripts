...
#if ! ${INSTX_TOPDIR}/build/build-libhsts.sh
#then
...
then
    if ! ${INSTX_TOPDIR}/build/build-flex.sh
    then
...
then
    if ! ${INSTX_TOPDIR}/build/build-grep.sh
    then
...
then
    if ! ${INSTX_TOPDIR}/build/build-lzip.sh
    then
...
if [[ -e ${INSTX_TOPDIR}/patch/wget2.patch ]]; then
    echo ""
    echo "**************************"
    echo "Patching Wget2"
    echo "**************************"

    patch -u -p0 < ${INSTX_TOPDIR}/patch/wget2.patch
fi
