...
if [[ "$IS_SOLARIS" -eq 1 ]]; then
    if ! ${INSTX_TOPDI${INSTX_TOPDIR}/build/build-boehm-gc.sh
    then
...
if ! ${INSTX_TOPDI${INSTX_TOPDIR}/build/build-libffi.sh
then
...
if [[ -e ${INSTX_TOPDIR}/build/build/guile3.patch ]]; then
    echo ""
    echo "***********************"
    echo "Patching package"
    echo "***********************"

    patch -u -p0 < ${INSTX_TOPDIR}/build/build/guile3.patch
fi
