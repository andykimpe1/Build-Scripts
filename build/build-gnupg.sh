...
if ! ${INSTX_TOPDIR}/build/build-gpgerror.sh
then
...
if ! ${INSTX_TOPDIR}/build/build-libksba.sh
then
...
if ! ${INSTX_TOPDIR}/build/build-libassuan.sh
then
...
if ! ${INSTX_TOPDIR}/build/build-libgcrypt.sh
then
...
if ! ${INSTX_TOPDIR}/build/build-ntbTLS.sh
then
...
if ! ${INSTX_TOPDIR}/build/build-nPth.sh
then
...
if [[ -e ${INSTX_TOPDIR}/patch/gnupg.patch ]]; then
    echo ""
    echo "**********************"
    echo "Patching package"
    echo "**********************"

    patch -u -p0 < ${INSTX_TOPDIR}/patch/gnupg.patch
fi
