...
if [[ -e ${INSTX_TOPDI${INSTX_TOPDIR}/build/build-cacerts.sh ]]; then
    BUILD_CACERTS=${INSTX_TOPDI${INSTX_TOPDIR}/build/build-cacerts.sh
elif [[ -e .${INSTX_TOPDI${INSTX_TOPDIR}/build/build-cacerts.sh ]]; then
    BUILD_CACERTS=.${INSTX_TOPDI${INSTX_TOPDIR}/build/build-cacerts.sh
elif [[ -e ../.${INSTX_TOPDI${INSTX_TOPDIR}/build/build-cacerts.sh ]]; then
    BUILD_CACERTS=../.${INSTX_TOPDI${INSTX_TOPDIR}/build/build-cacerts.sh
fi
...
if ! "${WGET}" -q -O "${PATCHELF_TAR}" \
     "https://github.com/Nix${INSTX_TOPDIR}/build/buildelf/releases/download/${PATCHELF_VER}/patchelf-${PATCHELF_VER}-${PATCHELF_ARCH}.tar.gz"
then
...
if [[ -n "${SUDO_PASSWORD}" ]]; then
    ...
    printf "%s\n" "${SUDO_PASSWORD}" | sudo cp -p b${INSTX_TOPDIR}/build/buildelf "${INSTX_PREFIX}/bin"
    printf "%s\n" "${SUDO_PASSWORD}" | sudo ${SUDO_ENV_OPT} -S bash "${INSTX_TOPDIR}/fix-permissions.sh" "${INSTX_PREFIX}"
else
    mkdir -p "${INSTX_PREFIX}/bin"
    cp -p b${INSTX_TOPDIR}/build/buildelf "${INSTX_PREFIX}/bin"
