...
if [[ -e ${INSTX_TOPDIR}/patch/gmp.patch ]]; then
    echo ""
    echo "*************************"
    echo "Patching package"
    echo "*************************"

    patch -u -p0 < ${INSTX_TOPDIR}/patch/gmp.patch
fi
...
if [[ "$OSX_10p5_OR_BELOW" -ne 0 ]]; then
    if [[ -e ${INSTX_TOPDIR}/patch/gmp-darwin.patch ]]; then
        echo ""
        echo "*************************"
        echo "Patching package (Darwin)"
        echo "*************************"

        patch -u -p0 < ${INSTX_TOPDIR}/patch/gmp-darwin.patch
    fi
fi
