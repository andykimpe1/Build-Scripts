...
if ! ${INSTX_TOPDIR}/build/build-cmake.sh
then
...
cd multicoreware${INSTX_TOPDIR}/build/build/linux




# Patches are created with 'diff -u' from the pkg root directory.
if [[ -e ${INSTX_TOPDIR}/patch/x265.patch ]]; then
    echo ""
    echo "**********************"
    echo "Patching package"
    echo "**********************"

    patch -u -p0 < ${INSTX_TOPDIR}/patch/x265.patch
fi
